...
public class Painter extends View {
    ...
    private ArrayList<ArrayList <Dots>> paths = new ArrayList<>();
    ...
    public void startCheckingDot(View v, boolean color) {
        System.out.println("okay lets go");
        for (int i = 0; i < 15; i++) {

            for (Dots dot : dotField[i]) {

                if (dot.x == v.getX() && dot.y == v.getY()) {
                    dot.player = color? 1:2;
                    startDot = dot;
                    checkDot(startDot);
                    break;
                }
    ...
    private void checkDot(Dots dot) {
        for (int i = 0; i < 15; i++) {
            for (Dots dotToCheck : dotField[i]) {
                if((dot.x + step == dotToCheck.x && dot.y == dotToCheck.y) && (dot.player == dotToCheck.player)){
                }
